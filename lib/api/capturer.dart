...
class Capturer extends _ComplexObject{
  ...
  List<RawSample> _capture(RawImageF image){
    final cap = _dll_handle.lookupFunction<_CapCap_c, _CapCap_dart>
      (_c_namespace + 'Capturer_capture_raw_image_with_crop');
    Pointer<Pointer<Void>> _emptyPointerStrList = malloc.allocate(sizeOf<Pointer<Pointer<Void>>>());
    int address_before = _emptyPointerStrList[0].address;
    ...
    cap(
      ...
      _emptyPointerStrList.cast(),
      Pointer.fromFunction(_assign_pointers_vector_func),
      exception
    );

    var rs = RawSample(_dll_handle, _emptyPointerStrList[0]);
    final destr = _dll_handle.lookupFunction<_objDestructor_c, _objDestructor_dart>(_c_namespace + 'apiObject_destructor');
    if (address_before != _emptyPointerStrList[0].address)
      destr(_emptyPointerStrList[0]);
